...
abstract contract BaseTokenBridge {
     ...
     */
    function bridgeERC20ToL2(address token, uint256 amount, uint256 gasLimit, address to) internal ValidSender OnlyIfEnoughPay returns(bool, uint256) {

        // pulling tokens from user
        IERC20(token).transferFrom(msg.sender, address(this), amount);
        // sending tokens to gateway router
        IERC20(token).approve(address(s_gatewayRouterL1), amount);
        s_gatewayRouterL1.depositERC20{value: msg.value}(
            token,
            to,
            amount,
            gasLimit
     ...
     * @dev a user needs to pay the gateway router for bridging ERC20 token L1/L2
     */
    function bridgeERC20ToL1(address token, uint256 amount, uint256 gasLimit, address to) internal ValidSender OnlyIfEnoughPay returns(bool, uint256) {

        IERC20(token).transferFrom(msg.sender, address(this), amount);
        // sending tokens to gateway router
        IERC20(token).approve(address(s_gatewayRouterL2), amount);
        s_gatewayRouterL2.withdrawERC20{value: msg.value}(
            token,
            to,
            amount,
            gasLimit
